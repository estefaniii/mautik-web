...
export class AnalyticsService {
	...
	// Track user activity
	static async trackUserActivity(userId: string) {
		try {
			const today = new Date();
			today.setHours(0, 0, 0, 0);
	...
	static async getDashboardAnalytics() {
		try {
			...
			const salesData = await prisma.orderAnalytics.findMany({
				where: {
					date: {
						gte: thirtyDaysAgo,
					},
			...
			const previousSalesData = await prisma.orderAnalytics.findMany({
				where: {
					date: {
						gte: sixtyDaysAgo,
						lt: thirtyDaysAgo,
			...
			const monthlySales = await prisma.orderAnalytics.groupBy({
				by: ['date'],
				where: {
					date: {
						gte: thirtyDaysAgo,
			...
			return {
				...
				topProducts: topProducts.map((product: any, index: number) => ({
					...product,
					product: products.find((p: any) => p.id === product.productId),
					rank: index + 1,
				})),
